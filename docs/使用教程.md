...
它和 [Clash Verge](https://github.com/clash-verge-rev/clash-verge-rev) 这类系统代理不一样：扩展会通过浏览器代理 API 给当前浏览器设置 PAC 代理，并自动回填代理鉴权。当前开放 `711Proxy` 与 `Webshare` 的账号密码模式，支持多账号列表；`API 拉取` 暂未开放。
...
在侧边栏打开 `IP代理` 后，可以在 `代理服务` 中选择 `711Proxy` 或 `Webshare`。
点击旁边的 `注册` / `登录` 按钮，按页面提示完成注册或登录。
