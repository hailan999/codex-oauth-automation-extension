...
test('Hotmail 本地助手验证码轮询有单次硬超时保护', () => {
  const background = fs.readFileSync('background.js', 'utf8');

  assert.match(
    background,
    /const HOTMAIL_LOCAL_CODE_HARD_TIMEOUT_MS = 20000;/,
    '本地助手验证码请求应有 20 秒硬超时常量'
  );
  assert.match(
    background,
    /runHotmailLocalCodeRequestWithHardTimeout\(workingAccount, pollPayload\)/,
    '步骤 4 Hotmail 本地助手轮询应通过硬超时包装调用 /code'
  );
  assert.match(
    background,
    /Hotmail 本地助手验证码请求硬超时/,
    '硬超时应给出可见错误，避免单次请求无日志卡住'
  );
});
