...
test('signup-only save session failure skips to the next round without auto retry enabled', async () => {
  const events = {
    logs: [],
    broadcasts: [],
    accountRecords: [],
  ...
  assert.equal(events.runCalls, 2, 'save-session failure should skip the failed round and start the next round');
  assert.equal(events.broadcasts.some(({ phase }) => phase === 'retrying'), false, 'save-session failure should not retry the same round');
  assert.equal(events.broadcasts.some(({ phase }) => phase === 'stopped'), false, 'save-session failure should not stop the whole auto-run');
  assert.equal(events.accountRecords.length, 1);
  assert.equal(events.accountRecords[0].status, 'failed');
  assert.match(events.accountRecords[0].reason, /api\/auth\/session/);
  assert.ok(events.logs.some(({ message }) => /保存 ChatGPT Session失败.*跳过到下一轮/.test(message)));
  assert.equal(runtime.state.autoRunActive, false);
  assert.equal(runtime.state.autoRunSessionId, 0);
});

test('signup-only non-final step failure skips to the next round after local retries are exhausted', async () => {
  const events = {
    logs: [],
    broadcasts: [],
    accountRecords: [],
  ...
  const controller = api.createAutoRunController({
    ...
    getStepDefinitionForState: (step) => ({ id: step, key: step === 2 ? 'submit-signup-email' : 'unknown' }),
    getStopRequested: () => false,
    hasSavedProgress: () => false,
    isAddPhoneAuthFailure: () => false,
    isRestartCurrentAttemptError: () => false,
    ...
    runAutoSequenceFromStep: async () => {
      ...
      if (events.runCalls === 1) {
        ...
        throw new Error('步骤 2：检测到当前停留在已登录 ChatGPT 首页。');
      }
    },
    runtime,
    setState: async (updates = {}) => {
  ...
  assert.equal(events.runCalls, 2, 'exhausted signup-only step failure should skip the failed round and start the next round');
  assert.equal(events.broadcasts.some(({ phase }) => phase === 'stopped'), false);
  assert.equal(events.accountRecords.length, 1);
  assert.equal(events.accountRecords[0].status, 'failed');
  assert.match(events.accountRecords[0].reason, /步骤 2/);
  assert.ok(events.logs.some(({ message }) => /当前步骤失败.*跳过到下一轮/.test(message)));
});
